...
) # do not change strings to factors
...
**2. For each of the five data-frame subsetting methods in the article above, subset**
   ...
   ***i. Your row***
```{r}
MyRaw1 <- myClass[(2), (2)]# extracts observations from Region 2 (raws from 10 to 21) and three variables: State (column 2), Minor.Population   #(column 6), and Education.Expenditures (Column 7)
MyRaw1
MyRaw2 <- myClass[-c(1,3,4), c(1:4)]
```

MyRaw2 <- [-c(1:9,22:50),-c(1,3:5)] # will give same results as ed_exp1 by ommitting raws and colums that we don't need (using negative vector)
ed_exp2
```{r}
MyRAw3 <- myClass[which(myClass== 2), names(myClass) %in% c("LName","FName","CafTeaCaps", "Neighb")]
myRaw3
